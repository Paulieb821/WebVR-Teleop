...
class IK_Reference_Generator(Node):
    ...
    def __init__(self):
        ...
        urdf_path = '4dof_arm_v2/4dof_arm_v2.xml'
        site_name = 'endeff'

        # Set Up UDRF
        dirname = os.path.dirname(__file__)
        abspath = os.path.join(dirname, '../../../robot_models', urdf_path)
        urdf_path = abspath
    ...
    def tracking_active_callback(self, track_act):
        self.tracking_active = track_act

    def ik_callback(self, controller_pos):
        pos_vec = np.array([controller_pos.x, controller_pos.y, controller_pos.z]).reshape(-1, 1)
